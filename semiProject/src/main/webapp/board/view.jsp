			...
			<div id="review_box">
				<form name="form1" method="post">
					<table id="board">
						...
						<tr>
							<td colspan="4"><c:if test="${dto.filesize>0}">
									<img src="${path}/board/review/${dto.filename}" width="300px"
										height="300px" style="border-radius: 20px;">
								</c:if></td>
						</tr>
						...
						<c:if test="${sessionScope.userid == dto.writer}">
							<tr>
								...
								<td colspan="3"><input type="password" name="passwd"
									class="form-control" required style="width: 150px;"> 
					...
					<table id="comment_board">
						...
						<tr>
							<td><input id="writer" name="writer" class="form-control"
								placeholder="작성자"></td>
							<td><input class="form-control" name="content"
								placeholder="내용을 입력하세요" id="content"></td>
							<td>
								<button type="button" id="btnSave"
									class="btn btn-primary btn-sm" onclick="comment_add()">등록</button>
							</td>
						</tr>
						<tr>
							<!-- 댓글 목록 -->
...
<%-- 										<c:forEach var="row" items="${list}">
											<tr>
												<td style="font-size: 13px;">${row.writer} 
													(<fmt:formatDate value="${row.reg_date}" pattern="yyyy-MM-dd hh:mm:ss" />)
												</td>
											</tr>
											<tr>
												<td
													style="font-size: 15px; border-bottom: 1px solid #eaeaea;">
													${row.content}</td>
											</tr>
										</c:forEach>
									</table> --%>
